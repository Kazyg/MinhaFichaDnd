...
export default function AbaMagias() {
  ...
  function calcularMagiasConhecidas() {
    ...
    if (ficha) {
      ...
      if (levelTotal) for (let nivel = 1; nivel <= levelTotal; nivel++) {
        ...
        if (classeNoNivel?.classe instanceof Bruxo) {
          magiasConhecidas.push({ classe: "Bruxo", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.magiasConhecidas ?? 0 });
          truqueConhecidos.push({ classe: "Bruxo", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.truquesConhecidos ?? 0 });
        }
        if (classeNoNivel?.classe instanceof Clerigo) {
          let magiaClerigo = 0
          let sabedoriaAtual = atributosPersonagem?.sabedoria.valor;
          const efeitosSabedoria = efeitos?.filter((e: any) => e.level === nivel && e.atributo === "sabedoria");
          ...
          truqueConhecidos.push({ classe: "Clerigo", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.truquesConhecidos ?? 0 });
        }
        if (classeNoNivel?.classe instanceof Druida) {
          let magiaDruida = 0
          let sabedoriaAtual = atributosPersonagem?.sabedoria.valor;
          const efeitosSabedoria = efeitos?.filter((e: any) => e.level === nivel && e.atributo === "sabedoria");
          if (efeitosSabedoria) if (efeitosSabedoria.length > 0) {
          ...
          truqueConhecidos.push({ classe: "Druida", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.truquesConhecidos ?? 0 });
        }
        if (classeNoNivel?.classe instanceof Feiticeiro) {
          magiasConhecidas.push({ classe: "Feiticeiro", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.magiasConhecidas ?? 0 });
          truqueConhecidos.push({ classe: "Feiticeiro", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.truquesConhecidos ?? 0 });
        }
        if (classeNoNivel?.classe instanceof Lutador) {
          let subClasse = ficha?.subClasse?.find(s => s.classe.nome === classeNoNivel.classe.nome)?.subclasse;
          if (subClasse instanceof CavaleiroArcano) {
            magiasConhecidas.push({ classe: "Lutador", magias: subClasse.magiasEspacos.find(m => m.nivel === nivel)?.magias ?? 0 });
            truqueConhecidos.push({ classe: "Lutador", magias: subClasse.magiasEspacos.find(m => m.nivel === nivel)?.truques ?? 0 });
          }
        }
        if (classeNoNivel?.classe instanceof Rogue) {
          let subClasse = ficha?.subClasse?.find(s => s.classe.nome === classeNoNivel.classe.nome)?.subclasse;
          if (subClasse instanceof TrapaceiroArcano) {
            magiasConhecidas.push({ classe: "Ladino", magias: subClasse.magiasEspacos.find(m => m.nivel === nivel)?.magias ?? 0 });
            truqueConhecidos.push({ classe: "Ladino", magias: subClasse.magiasEspacos.find(m => m.nivel === nivel)?.truques ?? 0 });
          }
        }
        if (classeNoNivel?.classe instanceof Mago) {
          let magiaMago = 0
          let inteligenciaAtual = atributosPersonagem?.sabedoria.valor;
          const efeitosInteligencia = efeitos?.filter((e: any) => e.level === nivel && e.atributo === "inteligência");
          ...
          truqueConhecidos.push({ classe: "Mago", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.truquesConhecidos ?? 0 });
        }
        if (classeNoNivel?.classe instanceof Paladino) {
          let magiaPaladino = 0
          let carismaAtual = atributosPersonagem?.sabedoria.valor;
          const efeitosCarisma = efeitos?.filter((e: any) => e.level === nivel && e.atributo === "carisma");
          ...
          magiasConhecidas.push({ classe: "Mago", magias: magiaPaladino });
          truqueConhecidos.push({ classe: "Mago", magias: 0 });
        }
        if (classeNoNivel?.classe instanceof Ranger) {
          magiasConhecidas.push({ classe: "Patrulheiro", magias: classeNoNivel.classe.niveis.find(n => n.nivel === nivel)?.magiasConhecidas ?? 0 });
  ...
  function validarMagiaClasse(magia: string) {
    ...
    } else if (!!magiasBruxo.find(m => m.nome === magia)) {
      classe = "Bruxo";
      magiaAchada = magiasBruxo.find(m => m.nome === magia);
    } else if (!!magiasClerigo.find(m => m.nome === magia)) {
      classe = "Clerigo";
      magiaAchada = magiasClerigo.find(m => m.nome === magia);
    } else if (!!magiasDruida.find(m => m.nome === magia)) {
      classe = "Druida";
      magiaAchada = magiasDruida.find(m => m.nome === magia);
    } else if (!!magiasFeiticeiro.find(m => m.nome === magia)) {
      classe = "Feiticeiro";
      magiaAchada = magiasFeiticeiro.find(m => m.nome === magia);
    } else if (!!magiasMago.find(m => m.nome === magia)) {
      classe = "Mago";
      magiaAchada = magiasMago.find(m => m.nome === magia);
    } else if (!!magiasPaladino.find(m => m.nome === magia)) {
      classe = "Paladino";
      magiaAchada = magiasPaladino.find(m => m.nome === magia);
    } else if (!!magiasPatrulheiro.find(m => m.nome === magia)) {
      classe = "Patrulheiro";
      magiaAchada = magiasPatrulheiro.find(m => m.nome === magia);
    }
    ficha?.setMagiaEscolhidas({ classe: classe, magia: magia });
  }
  ...
  return (
    <>
      <div key={refreshKey} className="magias">
        ...
        {ficha?.magiasEscolhidas?.map((magiaEscolhida) => (
          <>
            {magiaEscolhida.magia.map((magia, index) => {
              return (
                <div key={index} className="magia-container">
                  ...
                  <div className="dados-magia-container">
                    ...
                    <div className="magia-coluna2">
                      <button className="botao-equipar" onClick={() => { setModalInfoMagiasAberta(true); setMagiaInfo(magia) }}>
                        <img src={magiaIcon1} className="equipar" />
                        <img src={magiaIcon1} className="equipar-hover" />
                      </button>
                    </div>
                    <div className="magia-coluna3">
                      <button className="botao-equipar" onClick={() => handleUsarMagia(index)}>
                        <img src={magiaIcon1} className="equipar" />
                        <img src={magiaIcon1} className="equipar-hover" />
                      </button>
                      <button
                                ...
                                onClick={() => {
                                    ficha.excluirMagiaEscolhidas(magia)
                                    forceUpdate();
                                }}
